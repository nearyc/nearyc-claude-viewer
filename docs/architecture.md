# Claude Viewer - 产品级整合方案

## 产品定位

**Claude Viewer** = Sessions 历史查看器 + Agent Teams 监控器 + 数据可视化分析平台

不是简单的功能堆砌，而是建立两个模块之间的有机联系，提供完整的 Claude 使用洞察和高效的操作体验。

---

## 用户故事与场景

### 场景 1: 复盘一次 Agent Team 协作
> "昨天我用 swarming 模式让 3 个 agent 完成了一个功能，今天想复盘整个过程"

**用户路径**:
1. 进入 Dashboard，查看实时活动时间线
2. 找到昨天的 Session 活动记录
3. 点击打开，发现该 Session 关联了 "feature-dev-team"
4. 点击跳转，查看 Team 中每个成员的消息和状态变化
5. 使用 Cmd+K 快速切换到其他视图

### 场景 2: 监控正在运行的 Team
> "我启动了一个 Agent Team 在处理任务，想知道现在进展如何"

**用户路径**:
1. 进入 Agent Teams 视图
2. 查看实时消息流
3. 点击 "查看相关 Session" 跳转到完整的对话上下文
4. 导出重要 Session 为 Markdown 保存

### 场景 3: 分析使用习惯和产出
> "我想知道这周的代码产出和会话活跃度"

**用户路径**:
1. 进入 Dashboard 视图
2. 查看会话活跃度热力图，了解每天的活跃程度
3. 查看趋势图，分析会话数量变化
4. 查看代码产出统计，了解各语言代码行数

### 场景 4: 通过深度链接分享 Session
> "这个 Session 很重要，我想分享给同事"

**用户路径**:
1. 在 Sessions 视图中找到 Session
2. 复制浏览器地址栏的 URL（如 `/#/sessions/abc123`）
3. 分享给同事
4. 同事打开链接直接跳转到该 Session

### 场景 5: 使用命令面板快速导航
> "我想快速跳转到某个 Session"

**用户路径**:
1. 按 Cmd+K 打开命令面板
2. 输入 "Search Sessions"
3. 输入关键词搜索
4. 选择结果直接跳转

### 场景 6: 批量导出 Sessions
> "我需要导出本周的所有 Sessions 做备份"

**用户路径**:
1. 进入 Sessions 视图
2. 使用智能筛选选择本周的 Sessions
3. 批量选择多个 Session
4. 点击批量导出，选择格式

---

## 产品架构

### 主导航结构 (Sidebar)

```
┌─────────────────────────┐
│  🤖 Claude Viewer       │
├─────────────────────────┤
│                         │
│  📊 Dashboard           │
│  💬 Sessions        (86)│
│  📁 Projects        (12)│
│  👥 Agent Teams     (5) │
│                         │
├─────────────────────────┤
│  ⌘ Cmd+K 命令面板       │
├─────────────────────────┤
│  ⚡ Connected            │
└─────────────────────────┘
```

### 核心视图

| 视图 | 功能 | 布局 | 特色功能 |
|------|------|------|----------|
| **Dashboard** | 全局概览 + 数据可视化 | 仪表盘 | 热力图、趋势图、时间线 |
| **Sessions** | 会话列表 + 详情 | 三栏式 | 批量操作、标签、导出 |
| **Projects** | 项目筛选视图 | 三栏式 | 项目统计 |
| **Agent Teams** | Team 列表 + 成员 + 消息 | 三栏式 | 实时消息、状态监控 |

### 关键交互设计

#### 1. 全局命令面板 (Cmd+K)
```
┌────────────────────────────────────────┐
│ ⌘ Search commands...                   │
├────────────────────────────────────────┤
│  Navigation                            │
│  → Go to Dashboard           ⌘D        │
│  → Go to Sessions            ⌘S        │
│  → Go to Teams               ⌘T        │
│                                        │
│  Actions                               │
│  → Export Current Session              │
│  → Refresh Data                        │
│  → Toggle Theme                        │
└────────────────────────────────────────┘
```

#### 2. Dashboard 数据可视化
```
┌────────────────────────────────────────┐
│ 📊 Dashboard                            │
├────────────────────────────────────────┤
│ ┌────────┐ ┌────────┐ ┌────────┐       │
│ │Sessions│ │Projects│ │ Teams  │       │
│ │   86   │ │   12   │ │   5    │       │
│ └────────┘ └────────┘ └────────┘       │
│                                         │
│ ┌────────────────┐ ┌────────────────┐   │
│ │ Activity       │ │ Session Trend  │   │
│ │ Heatmap        │ │ (14 days)      │   │
│ │ [███░░███░]   │ │ /\/\/\         │   │
│ └────────────────┘ └────────────────┘   │
│                                         │
│ 📈 Real-time Activity Timeline          │
│ • Today                                │
│   [💬] Session: 修改PPT...    2h ago   │
│   [👥] Team: feature-dev-team 3h ago   │
│ • Yesterday                            │
│   [💬] Session: 优化代码...    1d ago   │
└────────────────────────────────────────┘
```

#### 3. Session → Team 跳转
在 SessionDetail 中添加关联 Team 信息：

```
┌────────────────────────────────────────┐
│ Session: 3df0892e...              [🔗] │
│ Project: vscodeTest                    │
│ Time: 2 hours ago                      │
│ Tags: [feature] [bugfix]               │
│                                        │
│ 👥 关联 Team: feature-dev-team    [查看] │
│                                        │
│ [导出] [在 Claude Code 中打开] [★收藏]   │
├────────────────────────────────────────┤
│ 消息列表...                             │
```

#### 4. 批量操作栏
```
┌────────────────────────────────────────┐
│ [✓] 已选择 5 个 Session                 │
│ [全选] [取消] [批量删除] [批量导出]      │
└────────────────────────────────────────┘
```

---

## 技术实现方案

### 整体架构

```
┌─────────────────────────────────────────────────────────────┐
│                        Frontend                              │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────────────┐   │
│  │  Dashboard  │ │  Sessions   │ │    Agent Teams      │   │
│  │  - Heatmap  │ │  - List     │ │    - List           │   │
│  │  - Trend    │ │  - Detail   │ │    - Members        │   │
│  │  - Timeline │ │  - Export   │ │    - Messages       │   │
│  └─────────────┘ └─────────────┘ └─────────────────────┘   │
│                                                              │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────────────┐   │
│  │CommandPalette│ │ useUrlState │ │   useSessionTags    │   │
│  │  (Cmd+K)    │ │ (Deep Link) │ │   (Tag System)      │   │
│  └─────────────┘ └─────────────┘ └─────────────────────┘   │
└──────────────────────────┬──────────────────────────────────┘
                           │ WebSocket / HTTP
┌──────────────────────────▼──────────────────────────────────┐
│                        Backend                               │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────────────┐   │
│  │   Routes    │ │  Services   │ │   FileWatcher       │   │
│  │  - sessions │ │  - sessions │ │   (Real-time)       │   │
│  │  - teams    │ │  - teams    │ └─────────────────────┘   │
│  │  - stats    │ │  - stats    │                           │
│  │  - search   │ │  - search   │ ┌─────────────────────┐   │
│  └─────────────┘ │  - code     │ │   ~/.claude/        │   │
│                  └─────────────┘ │   - history.jsonl   │   │
│                                  │   - teams/          │   │
│                  ┌─────────────┐ └─────────────────────┘   │
│                  │ Socket.io   │                           │
│                  │ (Events)    │                           │
│                  └─────────────┘                           │
└─────────────────────────────────────────────────────────────┘
```

### 数据关联设计

**Session 和 Team 关联**:
- 通过时间范围和项目路径自动关联
- Session Detail 中显示可能相关的 Team
- Team 消息中可跳转到关联 Session

**数据流分层**:
```
┌────────────────────────────────────────┐
│  Frontend (React + TypeScript)        │
│  - State Management: React Hooks      │
│  - UI: Tailwind CSS                   │
│  - Icons: Lucide React                │
├────────────────────────────────────────┤
│  API Layer (Express)                  │
│  - REST API                           │
│  - WebSocket Events                   │
├────────────────────────────────────────┤
│  Service Layer                        │
│  - Business Logic                     │
│  - Data Transformation                │
│  - Caching                            │
├────────────────────────────────────────┤
│  Data Source                          │
│  - File System (JSONL/JSON)           │
│  - FileWatcher (chokidar)             │
└────────────────────────────────────────┘
```

### 后端架构

```
claude-viewer/backend/
├── src/
│   ├── index.ts              # 服务器入口
│   ├── server.ts             # Express + Socket.io 配置
│   ├── types/
│   │   └── index.ts          # 统一类型定义
│   ├── routes/
│   │   ├── sessions.ts       # /api/sessions/*
│   │   ├── teams.ts          # /api/teams/*
│   │   ├── projects.ts       # /api/projects/*
│   │   ├── stats.ts          # /api/stats/* (活动、趋势、代码统计)
│   │   ├── search.ts         # /api/search (全文搜索)
│   │   └── execute.ts        # /api/execute (shell 命令)
│   ├── services/
│   │   ├── sessions/             # Sessions 模块化服务 (重构后)
│   │   │   ├── PathUtils.ts          # 路径处理工具函数
│   │   │   ├── SessionCache.ts       # 缓存管理 (get/set/clear/stats)
│   │   │   ├── SessionLoader.ts      # 从 history.jsonl 加载会话
│   │   │   ├── ProjectScanner.ts     # 扫描项目目录
│   │   │   ├── ConversationLoader.ts # 加载完整对话
│   │   │   ├── SessionRepository.ts  # 会话 CRUD 操作
│   │   │   └── __tests__/            # 单元测试 (70+)
│   │   ├── sessionsService.ts    # 精简后的主服务（组合模式）
│   │   ├── teamsService.ts
│   │   ├── statsService.ts       # 统计数据服务
│   │   ├── searchService.ts      # 全文搜索服务
│   │   ├── codeStatsService.ts   # 代码产出统计
│   │   └── activityService.ts    # 实时活动流
│   └── utils/                    # 工具函数
│       ├── apiResponse.ts        # 统一 API 响应格式
│       ├── pathUtils.ts          # 路径处理
│       └── dateUtils.ts          # 日期处理
```

**统一 FileWatcher**
监控三个数据源：
- `~/.claude/history.jsonl` (Sessions)
- `~/.claude/projects/**/*.jsonl` (Session details)
- `~/.claude/teams/**/*.json` (Teams)

**时间戳格式处理**

Claude Code 的 history.jsonl 使用 Unix 时间戳（秒级，10位数字），而 JavaScript 使用毫秒级时间戳（13位数字）。后端自动进行转换：

```typescript
// sessionsService.ts
const timestampMs = entry.timestamp > 10000000000
  ? entry.timestamp  // 已经是毫秒级
  : entry.timestamp * 1000;  // 秒级转毫秒级
```

这样可以确保前端显示的相对时间（如 "2h ago"）是正确的。

**Stats API 设计**
```
GET /api/stats/activity?days=90     # 活动热力图数据
GET /api/stats/projects/:path/trends # 项目趋势
GET /api/stats/usage                # 使用统计摘要
GET /api/stats/code-output          # 代码产出统计
```

**Search API 设计**
```
GET /api/search?q=keyword&type=session&limit=20
```

**WebSocket 事件扩展**
```
# 原有事件
sessions:initial, sessions:updated
session:data, session:updated
teams:initial, teams:updated
team:data, team:messages

# 新增事件
stats:updated          # 统计数据更新
activity:stream        # 实时活动流
```

### 前端架构

```
claude-viewer/frontend/
├── src/
│   ├── App.tsx                 # 主应用，路由控制
│   ├── types/
│   │   └── index.ts
│   ├── contexts/               # React Contexts
│   │   ├── SessionNamesContext.tsx
│   │   └── TeamNamesContext.tsx
│   ├── hooks/
│   │   ├── useWebSocket.ts     # 统一 WebSocket
│   │   ├── useSessions.ts
│   │   ├── useTeams.ts
│   │   ├── useSessionNames.ts  # Session 自定义名称
│   │   ├── useTeamNames.ts     # Team 自定义名称
│   │   ├── useSessionTags.ts   # Session 标签系统
│   │   ├── useUrlState.ts      # URL hash 状态管理
│   │   ├── useCommandPalette.ts # 命令面板状态
│   │   └── useSavedFilters.ts  # 保存的筛选条件
│   └── components/
│       ├── Layout.tsx          # 可调整布局
│       ├── Sidebar.tsx
│       ├── Dashboard.tsx       # 仪表盘
│       │   ├── ActivityHeatmap.tsx    # 活动热力图
│       │   ├── TrendChart.tsx         # 趋势图
│       │   └── ActivityTimeline.tsx   # 活动时间线
│       ├── CommandPalette.tsx  # 全局命令面板
│       ├── sessions/
│       │   ├── SessionList.tsx      # 批量操作、智能筛选
│       │   ├── SessionDetail/       # 模块化重构后 (1130行 → 180行)
│       │   │   ├── components/      # 子组件
│       │   │   │   ├── SessionHeader.tsx    # 标题、项目名、返回按钮
│       │   │   │   ├── SessionMeta.tsx      # Session ID、时间统计
│       │   │   │   ├── SessionActions.tsx   # 导出、删除、标签操作
│       │   │   │   ├── NavigationBar.tsx    # 滚动导航按钮组
│       │   │   │   ├── SearchBar.tsx        # 搜索输入和过滤
│       │   │   │   ├── BookmarksList.tsx    # 书签列表
│       │   │   │   ├── ConversationView.tsx # 消息列表渲染
│       │   │   │   └── RawInputsView.tsx    # 原始输入显示
│       │   │   ├── hooks/           # 自定义 Hooks
│       │   │   │   ├── useBookmarks.ts      # 书签状态管理
│       │   │   │   ├── useSearch.ts         # 搜索功能
│       │   │   │   └── useScrollNavigation.ts # 滚动导航
│       │   │   ├── utils/           # 工具函数
│       │   │   │   └── escapeRegExp.ts      # 正则转义
│       │   │   └── SessionDetail.tsx        # 精简主组件
│       │   ├── ExportDialog.tsx     # 导出对话框
│       │   ├── TagSelector.tsx      # 标签选择器
│       │   ├── SmartFilterBar.tsx   # 智能筛选栏
│       │   └── BatchActionBar.tsx   # 批量操作栏
│       └── teams/
│           ├── TeamList.tsx
│           ├── MemberList.tsx
│           └── MessagePanel.tsx
```

#### URL State 管理 (深度链接)

```typescript
// useUrlState.ts
interface UrlState {
  view: 'dashboard' | 'sessions' | 'projects' | 'teams';
  sessionId?: string;
  teamId?: string;
  project?: string;
}

// URL 格式:
// #/dashboard
// #/sessions
// #/sessions/:id
// #/teams/:id
```

#### Command Palette 架构

```typescript
interface Command {
  id: string;
  label: string;
  shortcut?: string;
  icon?: React.ReactNode;
  action: () => void;
  category: 'navigation' | 'action' | 'search';
}

// 31+ 个内置命令
// - 导航: Go to Dashboard/Sessions/Teams/Projects
// - 操作: Export, Refresh, Toggle Theme
// - 搜索: Search Sessions, Search Teams
```

#### 标签系统架构

```typescript
// useSessionTags.ts
interface SessionTags {
  [sessionId: string]: string[];
}

const STORAGE_KEY = 'claude-viewer-session-tags';
// 使用 localStorage 持久化

// API:
// - getTags(sessionId): string[]
// - addTag(sessionId, tag): void
// - removeTag(sessionId, tag): void
// - getAllTags(): string[] (所有唯一标签)
```

#### 导出功能架构

```typescript
// 支持格式
enum ExportFormat {
  MARKDOWN = 'md',
  JSON = 'json',
  HTML = 'html'
}

// Markdown 格式
// # Session: {sessionId}
// - Project: {project}
// - Created: {createdAt}
//
// ## Messages
// ### User ({timestamp})
// {content}
// ### Assistant ({timestamp})
// {content}
```

#### 可视化组件架构

**ActivityHeatmap (活动热力图)**:
- 90 天数据
- 5 级活跃度 (0-4)
- 纯 CSS Grid 实现
- Tooltip 显示日期和数量

**TrendChart (趋势图)**:
- 14 天数据
- SVG 绘制
- 渐变填充
- 网格线和坐标轴

**ActivityTimeline (活动时间线)**:
- 按时间分组 (今天/昨天/本周/更早)
- Session (蓝色) + Team (紫色)
- 实时更新

### 测试架构

#### 后端测试 (Jest)

```
backend/
├── jest.config.js              # Jest 配置
├── test/
│   └── setup.ts                # 测试环境设置
└── src/
    ├── utils/__tests__/        # 工具函数测试
    │   └── pathUtils.test.ts
    └── services/sessions/__tests__/  # Sessions 服务测试
        ├── PathUtils.test.ts
        ├── SessionCache.test.ts
        ├── SessionLoader.test.ts
        ├── ProjectScanner.test.ts
        ├── ConversationLoader.test.ts
        └── SessionRepository.test.ts
```

**测试策略**:
- 单元测试覆盖所有服务模块
- 使用 `__tests__` 目录组织测试文件
- 目标覆盖率: 80%+

#### 前端测试 (Vitest)

```
frontend/
├── vitest.config.ts            # Vitest 配置
├── test/
│   └── setup.ts                # 测试环境设置
└── src/
    ├── utils/__tests__/        # 工具函数测试
    │   ├── exportUtils.test.ts
    │   └── time.test.ts
    └── components/SessionDetail/
        ├── components/__tests__/   # 组件测试
        │   ├── SessionHeader.test.tsx
        │   ├── SessionMeta.test.tsx
        │   └── ...
        └── hooks/__tests__/        # Hooks 测试
            ├── useBookmarks.test.ts
            ├── useSearch.test.ts
            └── useScrollNavigation.test.ts
```

**测试策略**:
- 组件测试使用 React Testing Library
- Hooks 测试使用 renderHook
- 工具函数独立测试

### 统计数据流

```
~/.claude/history.jsonl
    ↓ (FileWatcher)
backend/src/services/statsService.ts
    ↓ (统计分析)
├─ getActivityStats(days) → ActivityData[]
│   └─ { date: string, count: number, level: 0-4 }
├─ getProjectTrends(path, days) → TrendData[]
│   └─ { label: string, count: number }
├─ getUsageStats() → UsageStats
│   └─ { totalSessions, totalProjects, ... }
└─ getCodeStats(days) → CodeStats
    └─ { languages: { [lang]: lines }, totalLines }

    ↓ (HTTP / WebSocket)
frontend/src/components/
├─ ActivityHeatmap.tsx
├─ TrendChart.tsx
└─ ActivityTimeline.tsx
```

### 搜索数据流

```
~/.claude/history.jsonl
    ↓ (FileWatcher + Index)
backend/src/services/searchService.ts
    ↓ (全文索引)
GET /api/search?q=keyword
    ↓
frontend/src/components/SearchResults.tsx
```

---

## 实施阶段规划

### 阶段一: 基础设施增强 ✓

**任务列表**:
1. **面包屑导航+深度链接**
   - useUrlState.ts hook
   - URL hash 路由管理
   - 浏览器历史集成

2. **可折叠面板布局**
   - ResizablePanel 组件
   - 拖拽调整宽度
   - 面板折叠/展开

### 阶段二: 数据导出与标签 ✓

**任务列表**:
1. **会话导出功能**
   - ExportDialog 组件
   - Markdown/JSON/HTML 格式
   - SessionDetail 集成

2. **标签系统**
   - useSessionTags hook
   - TagSelector 组件
   - 按标签筛选

3. **智能筛选栏**
   - SmartFilterBar 组件
   - 保存/加载筛选条件

### 阶段三: 全局交互增强 ✓

**任务列表**:
1. **全局命令面板 (Cmd+K)**
   - CommandPalette 组件
   - 31+ 个命令
   - 键盘导航

2. **批量操作**
   - SessionList 多选
   - BatchActionBar 组件
   - 批量删除/导出

### 阶段四: 可视化仪表盘 ✓

**任务列表**:
1. **会话活跃度热力图**
   - ActivityHeatmap 组件
   - 90天活动数据
   - 5级颜色深度

2. **项目活跃度趋势图**
   - TrendChart 组件
   - 14天折线图
   - SVG 实现

3. **实时活动流**
   - ActivityTimeline 组件
   - 混合 Session + Team 活动
   - 按时间分组

4. **使用统计与分析**
   - Dashboard 扩展
   - statsService 服务
   - 代码产出统计

### 阶段五: 高级功能 ✓

**任务列表**:
1. **增强搜索**
   - searchService 服务
   - 全文搜索 API
   - 搜索结果页面

2. **代码产出统计**
   - codeStatsService 服务
   - 代码块解析
   - 语言分布统计

---

## Agent Teams 任务分配

### 蜂群团队结构

```
claude-viewer-swarm/
├── frontend-alpha      # 前端基础设施 + 全局交互
├── frontend-beta       # 前端可视化 + 仪表盘
├── backend-alpha       # 后端 API + 统计服务
├── backend-beta        # 后端搜索 + 代码统计
└── qa-tester          # 测试验证
```

### 详细分工

#### Frontend Alpha
**职责**: 基础设施 + 全局交互

**任务**:
- useUrlState.ts (深度链接)
- ResizablePanel.tsx (可折叠面板)
- CommandPalette.tsx (Cmd+K)
- BatchActionBar.tsx (批量操作)

#### Frontend Beta
**职责**: 数据导出 + 标签 + 可视化

**任务**:
- ExportDialog.tsx (导出)
- useSessionTags.ts + TagSelector.tsx (标签)
- SmartFilterBar.tsx (智能筛选)
- ActivityHeatmap.tsx (热力图)
- TrendChart.tsx (趋势图)
- ActivityTimeline.tsx (时间线)

#### Backend Alpha
**职责**: 统计 API + 活动流

**任务**:
- statsService.ts (活动统计)
- activityService.ts (实时活动)
- /api/stats/* 路由
- WebSocket activity:stream 事件

#### Backend Beta
**职责**: 全文搜索 + 代码统计

**任务**:
- searchService.ts (全文搜索)
- codeStatsService.ts (代码统计)
- /api/search 路由
- /api/stats/code-output 路由

---

## UI/UX 设计要点

### 1. 一致的视觉风格
- 深色主题 (Dark Theme)
- 统一使用 Tailwind CSS 配色
- 保持图标风格一致 (Lucide React)

### 2. 键盘优先
- Cmd+K 命令面板
- 快捷键支持
- 键盘导航

### 3. 深度链接
- 所有视图可分享
- URL hash 路由
- 浏览器历史集成

### 4. 实时反馈
- WebSocket 实时更新
- 操作成功提示
- 加载状态指示

### 5. 可访问性
- 足够的颜色对比度
- 键盘可访问
- 屏幕阅读器支持

---

## 配置参数

```bash
# 后端 (端口 13927)
PORT=13927
CLAUDE_DIR=~/.claude
HISTORY_FILE_PATH=~/.claude/history.jsonl
TEAMS_DIR=~/.claude/teams
CORS_ORIGIN=http://localhost:5173

# 前端 (端口 5173)
VITE_API_URL=http://localhost:13927
VITE_WS_URL=http://localhost:13927
```

---

## 性能优化

### 前端优化
- 虚拟列表 (大量 Session 时)
- useMemo/useCallback 缓存
- 懒加载非关键组件

### 后端优化
- 内存缓存 (文件修改时间检查)
- 增量更新 (WebSocket)
- 分页加载 (API limit)

### 文件监控优化
- 统一的 FileWatcher
- 防抖处理
- 只监控必要文件

---

## 后续迭代方向

1. **数据持久化**: 将标签、自定义名称同步到云端
2. **插件系统**: 支持第三方扩展
3. **主题系统**: 支持 Light/Dark 主题切换
4. **移动端适配**: 响应式布局优化
5. **AI分析**: 智能会话总结、关键词提取
6. **团队协作**: 多人共享收藏、评论
7. **集成更多数据源**: MCP 服务器监控等

---

## Teams 数据推断机制

### 问题背景
Claude Code 创建的 Agent Teams 通常只有 `inboxes` 数据而没有 `config.json` 配置文件，导致 Viewer 无法显示这些团队。

### 解决方案
`TeamsService` 实现了智能数据推断：

1. **优先读取 config.json**: 如果存在配置文件，使用配置中的团队信息
2. **自动推断模式**: 当配置文件不存在时，从 `inboxes/*.json` 文件推断：
   - **成员列表**: 每个 inbox 文件对应一个成员
   - **消息统计**: 统计每个 inbox 的消息数量
   - **时间戳**: 提取最早消息的时间作为团队创建时间
   - **Agent 类型**: 根据消息内容推断（explore/research → Explore, plan/design → Plan）
   - **团队名称**: 将目录名 `dsa-dev` 转换为 `Dsa Dev`

### 数据流示例
```
~/.claude/teams/dsa-dev/
├── inboxes/
│   ├── team-lead.json      → 成员: team-lead
│   ├── backend-dev.json    → 成员: backend-dev
│   └── frontend-dev.json   → 成员: frontend-dev
└── (无 config.json)

推断结果:
- 团队名称: "Dsa Dev"
- 成员数: 3
- 消息数: 各 inbox 消息总和
- 类型: team-lead (如果存在 team-lead 成员)
```

---

## 风险与缓解

| 风险 | 影响 | 缓解方案 |
|------|------|----------|
| 文件系统性能瓶颈 | 中 | 使用缓存机制，避免频繁读取 |
| 状态管理复杂化 | 中 | 保持单一数据源，使用不可变更新 |
| 浏览器兼容性 | 低 | 使用标准 Web API，避免实验性功能 |
| 内存泄漏 | 中 | 正确清理 WebSocket 订阅和定时器 |
| URL hash 冲突 | 低 | 规范化的 hash 格式，避免特殊字符 |

---

## 重构里程碑 (2026-02-18)

### 阶段六: 模块化架构重构 ✓

采用**蜂群并行开发模式**完成以下重构：

#### 后端服务拆分

**问题**: `sessionsService.ts` 676 行单体服务，职责混杂

**解决方案**: 拆分为 6 个独立模块

```
services/sessions/
├── PathUtils.ts          # 路径处理工具函数 (normalizePath, generateProjectSlug)
├── SessionCache.ts       # 缓存管理 (get/set/clear/stats)
├── SessionLoader.ts      # 从 history.jsonl 加载会话
├── ProjectScanner.ts     # 扫描项目目录
├── ConversationLoader.ts # 加载完整对话
├── SessionRepository.ts  # 会话 CRUD 操作
└── __tests__/            # 70+ 单元测试

sessionsService.ts        # 精简主服务（组合模式，向后兼容）
```

**成果**:
- 代码行数: 676 行 → 180 行（主服务）
- 职责分离：每个模块单一职责
- 可测试性：新增 70+ 单元测试
- 向后兼容：API 保持不变

#### 前端组件拆分

**问题**: `SessionDetail.tsx` 1130 行大组件

**解决方案**: 提取子组件和自定义 Hooks

```
components/SessionDetail/
├── components/
│   ├── SessionHeader.tsx      # 会话头部（标题、项目名）
│   ├── SessionMeta.tsx        # 元数据（ID、时间统计）
│   ├── SessionActions.tsx     # 操作按钮（导出、删除、收藏）
│   ├── NavigationBar.tsx      # 导航栏（滚动导航）
│   ├── SearchBar.tsx          # 搜索栏
│   ├── BookmarksList.tsx      # 书签列表
│   ├── ConversationView.tsx   # 对话视图
│   └── RawInputsView.tsx      # 原始输入视图
├── hooks/
│   ├── useBookmarks.ts        # 书签状态管理
│   ├── useSearch.ts           # 搜索功能（关键词、过滤、高亮）
│   └── useScrollNavigation.ts # 滚动导航逻辑
└── SessionDetail.tsx          # 精简主组件（~180 行）
```

**成果**:
- 代码行数: 1130 行 → 180 行
- 子组件复用：8 个独立组件
- Hooks 复用：3 个自定义 Hooks
- 测试覆盖：新增组件和 Hooks 测试

#### 性能优化

**App.tsx 优化**:
- 5 个独立的 useEffect → 合并为 1 个
- 使用 useMemo 缓存计算结果
- 优化依赖数组减少重渲染

**搜索性能优化**:
- useSearch.ts 使用 useMemo 缓存搜索结果
- 添加 useDeferredValue 延迟非紧急更新

#### 测试增强

**后端测试** (Jest):
- PathUtils, SessionCache, SessionLoader
- ProjectScanner, ConversationLoader, SessionRepository
- 总计 70+ 测试，覆盖率 80%+

**前端测试** (Vitest):
- exportUtils.ts, time.ts 工具函数
- SessionDetail 子组件和 Hooks
- 总计 73 个测试

**E2E 测试** (Playwright + Chrome DevTools):
- 会话列表加载（127 个会话）
- 会话详情查看
- 搜索功能验证
- 导出按钮可用性
- 导航功能验证

#### 蜂群开发模式

```
团队: claude-viewer-dev
├── backend-splitter   # 后端服务拆分
├── frontend-splitter  # 前端组件拆分
├── test-writer        # 工具函数测试
├── perf-optimizer     # 性能优化
└── e2e-tester         # E2E 测试
```

**并行执行**: 5 个 Agent 同时工作，总耗时 < 30 分钟

---

## Teams 功能重构 (2026-02-19)

### 问题背景

Agent Teams 模块存在以下问题：
1. **消息不实时显示**: WebSocket 消息更新未正确反映到 UI
2. **状态显示异常**: 只有 team-lead 显示状态，其他成员状态缺失
3. **布局问题**: 消息面板下方有大量空白区域
4. **交互问题**: 最新消息闪烁动画影响阅读

### 解决方案

#### 1. 消息实时同步修复

**问题**: `App.tsx` 缺少 `onMessagesUpdated` 回调

```typescript
// App.tsx - 新增消息更新处理
const handleMessagesUpdated = useCallback((data: {
  teamId: string;
  memberId: string;
  messages: Message[]
}) => {
  setTeamData((prev) => {
    if (!prev || prev.id !== data.teamId) return prev;
    return {
      ...prev,
      inboxes: prev.inboxes.map((inbox) =>
        inbox.memberName === data.memberId
          ? { ...inbox, messages: data.messages }
          : inbox
      ),
    };
  });
}, []);
```

#### 2. 成员状态检测增强

**文件**: `MemberList.tsx`

```typescript
// 从消息历史中解析成员当前状态
const getMemberStatusFromMessages = (messages: Message[]): string => {
  if (!messages || messages.length === 0) return 'offline';

  for (let i = messages.length - 1; i >= 0; i--) {
    const msg = messages[i];

    if (msg.type === 'idle_notification') {
      try {
        const data = JSON.parse(msg.content);
        return data.idleReason || data.status || 'available';
      } catch {
        return 'available';
      }
    }

    if (msg.type === 'status_update') {
      try {
        const data = JSON.parse(msg.content);
        return data.status || 'busy';
      } catch {
        return 'busy';
      }
    }
  }

  return 'online';
};
```

**状态类型映射**:
- `available`, `idle`, `completed`, `success` → 绿色
- `busy`, `working`, `processing` → 琥珀色
- `offline`, `shutdown` → 灰色
- `error`, `failed` → 红色

#### 3. 文件名与成员名映射修复

**问题**: Inbox 文件名为 `----E.json` 但 config.json 中成员名为 `策划师-E`

**修复**: `TeamsService.ts` 中添加模糊匹配

```typescript
private async findInboxFile(inboxesDir: string, memberName: string): Promise<string | null> {
  // 1. 精确匹配
  const exactPath = path.join(inboxesDir, `${memberName}.json`);
  if (await fileExists(exactPath)) return exactPath;

  // 2. 尝试各种变换
  const variations = [
    memberName.replace(/^策划师-/, '----'), // 策划师-E → ----E
    memberName.replace(/^----/, '策划师-'), // ----E → 策划师-E
    // ... 其他变体
  ];

  for (const variant of variations) {
    const variantPath = path.join(inboxesDir, `${variant}.json`);
    if (await fileExists(variantPath)) return variantPath;
  }

  return null;
}
```

#### 4. 轮询备份机制

**新增文件**: `useTeamPolling.ts`

```typescript
export function useTeamPolling({ interval = 5000, enabled = true, onTick }: PollingOptions) {
  const intervalRef = useRef<NodeJS.Timeout | null>(null);

  useEffect(() => {
    if (!enabled) return;

    // 立即执行一次
    onTickRef.current();

    // 设置轮询
    intervalRef.current = setInterval(() => {
      onTickRef.current();
    }, interval);

    return () => clearInterval(intervalRef.current);
  }, [interval, enabled]);

  return { refresh };
}
```

#### 5. 布局修复

**MessagePanel.tsx**:
```typescript
// 修复前: 固定高度限制
<div style={{ maxHeight: 'calc(100vh - 400px)', overflowY: 'auto' }}>

// 修复后: 自适应填充
<div className="flex flex-col h-full">
  <div className="flex-1 overflow-y-auto">
```

### 蜂群修复团队

```
团队: teams-fix-dev
├── bug-analyzer       # 问题分析
├── backend-fixer      # 后端修复
├── frontend-fixer     # 前端修复
└── ui-polisher        # UI 优化
```

---

## 模块设计原则

### 1. 单一职责
每个模块只负责一个功能：
- `SessionCache` 只管理缓存，不处理文件 I/O
- `SessionLoader` 只加载数据，不管理缓存
- `PathUtils` 只处理路径，不处理业务逻辑

### 2. 依赖注入
模块通过构造函数接收依赖：
```typescript
class SessionLoader {
  constructor(private pathUtils: PathUtils) {}
}
```

### 3. 接口隔离
每个模块暴露最小接口：
```typescript
interface ISessionCache {
  get(key: string): CacheEntry | undefined;
  set(key: string, value: CacheEntry): void;
  clear(): void;
}
```

### 4. 可测试性
- 每个模块独立测试
- Mock 外部依赖
- 边界条件全覆盖
